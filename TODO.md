...
Last updated: 2026-03-04 (session 3: supply.py done; testing infrastructure added)
...
### 4.5. Testing infrastructure + rule-oracle fixture generation

**Philosophy:** Fixtures are generated by a rules-oracle agent that reads only `cna_rules.txt` and
has never seen `src/`. The oracle produces JSON files defining inputs and expected outputs, with
exact PDF rule citations. The test runner is deterministic Python (no LLM at test time). Fixtures
are the oracle — if a fixture and the code disagree, check the PDF.

**Workflow for all future engine modules:** generate fixtures BEFORE writing engine code.

```
tests/
  fixtures/
    fuel_evaporation_49_3.json    — oracle-generated, rule 49.3
    pasta_rule_52_6.json          — oracle-generated, rule 52.6
    prisoner_stores_28_15.json    — oracle-generated, rule 28.15
    supply_line_32_16.json        — oracle-generated, rule 32.16
    movement_costs_8_37.json      — oracle-generated, rule 8.37
    zoc_10_11_10_21.json          — oracle-generated, rules 10.11 / 10.21
  conftest.py                     — fixture loader helpers
  test_supply.py                  — loads supply fixtures, calls engine, asserts
  test_hex_map.py                 — loads hex_map fixtures, calls engine, asserts
```

- [ ] Create `tests/` structure and `conftest.py`
- [ ] Oracle agent: supply module fixtures (rules 32.16, 49.3, 52.6, 28.15)
- [ ] Oracle agent: hex_map module fixtures (rules 8.37, 10.11, 10.21, 8.42)
- [ ] Write `test_supply.py` + `test_hex_map.py`
- [ ] All tests green; any fixture/code disagreement resolved against PDF
